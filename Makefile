...
install-semodule: install-config $(BASE_POLICY_SOURCES) $(STANDARD_POLICY_SOURCES)
	$(MKDIR) -p $(DESTDIR)/$(SHAREDSTATEDIR)/selinux/$(POLICY_NAME)
	$(SEMODULE) --priority=100 -i $(BASE_POLICY_SOURCES) $(STANDARD_POLICY_SOURCES) -N -s $(POLICY_NAME) -p $(DESTDIR)

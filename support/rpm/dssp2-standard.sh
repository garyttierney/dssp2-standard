...
{
    MINHEAD=`/bin/curl -s \
        https://api.github.com/repos/defensec/dssp2-standard/git/refs/heads/master \
    ...
    /bin/test -e ${HOME}/rpmbuild/SOURCES/dssp2-standard-${MINHEAD}.tar.gz \
        || /bin/wget -q \
            https://github.com/defensec/dssp2-standard/archive/${MINHEAD}/dssp2-standard-${MINHEAD}.tar.gz \
                -O ${HOME}/rpmbuild/SOURCES/dssp2-standard-${MINHEAD}.tar.gz
    ...
    /bin/test -e ${HOME}/rpmbuild/SPECS/dssp2-standard.spec \
        || /bin/wget -q \
            https://raw.githubusercontent.com/defensec/dssp2-standard/master/support/rpm/dssp2-standard.spec \
                -O ${HOME}/rpmbuild/SPECS/dssp2-standard.spec

    /bin/sed -i \
        "s/%global commit0.*/%global commit0 $MINHEAD/" \
            ${HOME}/rpmbuild/SPECS/dssp2-standard.spec

    /bin/sed -i \
        "s/%global commit1.*/%global commit1 $BASEHEAD/" \
            ${HOME}/rpmbuild/SPECS/dssp2-standard.spec

    /bin/rpmbuild -ba ${HOME}/rpmbuild/SPECS/dssp2-standard.spec
}
